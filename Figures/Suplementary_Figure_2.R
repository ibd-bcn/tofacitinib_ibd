...
corr_tofa2 <- read_excel("Figures/extra_data/Supporting data values Melon-Ardanaz et al.xlsx",
                         sheet = "Sup figure 2B")

colnames(corr_tofa2) <- gsub(" \\(AU\\)","",colnames(corr_tofa2))
colnames(corr_tofa2) <- gsub("Tofa concentration \\(ng/ml\\)","Conc tofa (ng/ml)",colnames(corr_tofa2))


corr_tofa2 <- corr_tofa2 %>%
  mutate(
    `Conc tofa (ng/ml)` = as.numeric(`Conc tofa (ng/ml)`),
    SOCS1 = as.numeric(SOCS1),
    SOCS3 = as.numeric(SOCS3),
    IRF1 = as.numeric(IRF1)
  )

# SOCS1 Plot
SOCS1 <- ggplot(corr_tofa2, aes(`Conc tofa (ng/ml)`, SOCS1)) +
  geom_point(aes(color = Response), size = 1) +
  geom_smooth(formula = y ~ log(x), method = 'lm', se = TRUE) +
  scale_color_manual(values = c('R' = '#70ADE6', 'NR' = '#FF8E47')) +
  labs(x = 'Tofa conc (ng/ml)', y = 'SOCS1') +
  theme_classic() +
  theme(legend.position = 'top', text = element_text(family = 'Helvetica', size = 8))

# SOCS3 Plot
SOCS3 <- ggplot(corr_tofa2, aes(`Conc tofa (ng/ml)`, SOCS3)) +
  geom_point(aes(color = Response), size = 1) +
  geom_smooth(formula = y ~ log(x), method = 'lm', se = TRUE) +
  scale_color_manual(values = c('R' = '#70ADE6', 'NR' = '#FF8E47')) +
  labs(x = 'Tofa conc (ng/ml)', y = 'SOCS3') +
  theme_classic() +
  theme(legend.position = 'top', text = element_text(family = 'Helvetica', size = 8))

# IRF1 Plot
IRF1 <- ggplot(corr_tofa2, aes(`Conc tofa (ng/ml)`, IRF1)) +
  geom_point(aes(color = Response), size = 1) +
  geom_smooth(formula = y ~ log(x), method = 'lm', se = TRUE) +
  scale_color_manual(values = c('R' = '#70ADE6', 'NR' = '#FF8E47')) +
  labs(x = 'Tofa conc (ng/ml)', y = 'IRF1') +
  theme_classic() +
  theme(legend.position = 'top', text = element_text(family = 'Helvetica', size = 8))
...
ggsave("combined_plot_tofa2B.png", plot = combined_plot, width = 8, height = 2.5, units = "in")

## Supplementary Figure 2C : Correlations ---------------------------------------
corr_tofa2 <- read_excel("Figures/extra_data/Supporting data values Melon-Ardanaz et al.xlsx",
                        sheet = "Sup figure 2C")
...
ggsave("combined_plot_tofa2C.png", plot = combined_plot, width = 8, height = 2.5, units = "in")
