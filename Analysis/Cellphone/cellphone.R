...
obtain_cell_phone <- function(directory,pvalues,significant_means){
  ...
  extract_cell_interactions <- extract_cell_interactions[13:length(extract_cell_interactions)]
  ...
  for(x in 1:length(extract_cell_interactions)){
    ...
    information <- significant_means[1:12] #Grab the 12 first columns where all the information is found
    interested_column <- significant_means[,12+x] #Grab the column of the interaction that you are interested with
    name_of_column <- extract_cell_interactions[x] #Extract the name of the interaction
    information[[name_of_column]] <- interested_column #Add to the information dataframe, the column of the interested interaction
    inf_df <- information[complete.cases(information[[name_of_column]]), ] #Delete all the NA rows in the column of the interested interaction
    #Number of rows
    row_number <- nrow(inf_df) #If the number of rows is 0, then go next, because no interaction found
    if(row_number == 0){
      next
    }
    ...
    for(p in 1:length(row_number)){
      y <- y + 1
      word_cell <- as.list(strsplit(extract_cell_interactions[x], ".", fixed = TRUE))[[1]]
      ...
      #pvalue
      pvalue_interaction <- pvalues[pvalues$id_cp_interaction == inf_df[p,1],extract_cell_interactions[x]]
...
fuse_all_cellphones <- function(cell_phone_group1,cell_phone_group2,cell_phone_group3,cell_phone_group4){
  dataframe <- data.frame(matrix(ncol = 28, nrow = 1))
  colnames_df <- colnames(cell_phone_group4)
  colnames_df <- c(colnames_df[1:16],"mean1","mean2","mean3","mean4","pvalue1","pvalue2","pvalue3","pvalue4","log10p1","log10p2","log10p3","log10p4")
  ...
  dataframe_names <- c("cell_phone_group2", "cell_phone_group3", "cell_phone_group4")
  ...
  for(name in dataframe_names){
    ...
    for(x in 1:nrow(p)){
      ...
      coincidence4 <- p[x,4]
      coincidence5 <- p[x,5]
      filtered_df <- subset(dataframe,  dataframe$Cluster_1== coincidence1 & dataframe$Cluster_2 == coincidence2 & dataframe$Subset_1 == coincidence3 & dataframe$Subset_2 == coincidence4 & dataframe$id_cp_interaction == coincidence5)
      number_rows <- as.numeric(nrow(filtered_df))
      if(number_rows != 0){
        condition <- dataframe$Cluster_1 == coincidence1 & dataframe$Cluster_2 == coincidence2 &
...
#Obtain genes
dummydata <- read10xCounts('/home/acorraliza/TOFA_data/output_cellranger/TOF-005-W0/')
...
tcells$annotation_intermediate <- gsub("HS","HS_tcells", tcells$annotation_intermediate)
tcells$annotation_intermediate <- gsub("Naïve T cells","Naive_T_cells", tcells$annotation_intermediate)
cycling$annotation_intermediate <- gsub("Cycling_B_cell","Cycling_B_cell_cycling", cycling$annotation_intermediate)
plasmas$annotation_intermediate <- gsub("Cycling_B_cell","Cycling_B_cell_plasmas", plasmas$annotation_intermediate)
...
for(i in 1:length(thelist)){
  ...
  comparison <- paste(thelist[[i]][1], thelist[[i]][2], sep = '_and_')
  #Rename metadata
  ...
  count_norm$Gene <- suppressWarnings(mapvalues(rownames(count_norm), geneinfo$Symbol, geneinfo$ID))
  count_norm <- count_norm[,c(ncol(count_norm), 1:(ncol(count_norm) -1))]
...
sample_type[] <- lapply(sample_type, function(x) gsub("-", "_", x))
...
for(x in 1:length(thelist)){
  ...
  cell1 <-thelist[[x]][1]
  cell2 <-thelist[[x]][2]
  path1 <- paste("~/TOFA/plots/cellphone_output/raw/",cell1,"_and_",cell2,"/cellphonedb_count.txt",sep = "")
  path2 <- paste("~/TOFA/plots/cellphone_output/raw/",cell1,"_and_",cell2,"/cellphonedb_meta.txt",sep = "")
  ...
  #Create directories
  dir.create(paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group1",sep=""))
  dir.create(paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group2",sep=""))
  dir.create(paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group3",sep=""))
  dir.create(paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group4",sep=""))
  ...
  #Dataframe group_1
  finding1 <- paste(group1_sample[1],"|",group1_sample[2],"|",group1_sample[3],"|",group1_sample[4],"|",group1_sample[5],"|Gene",sep = "")
  db_count_colnames_group1 <- db_count_colnames[grepl(finding1,db_count_colnames)]
  selected_db_count1 <- db_count[db_count_colnames_group1]
  selected_db_meta1 <- db_meta[db_meta$Cell %in%  db_count_colnames_group1[2:length(db_count_colnames_group1)],]
  write.table(selected_db_meta1, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group1/cellphonedb_meta.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)
  write.table(selected_db_count1, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group1/cellphonedb_count.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)

  #Dataframe group_2
  finding2 <- paste(group2_sample[1],"|",group2_sample[2],"|",group2_sample[3],"|",group2_sample[4],"|",group2_sample[5],"|",group2_sample[6],"|Gene",sep = "")
  db_count_colnames_group2 <- db_count_colnames[grepl(finding2,db_count_colnames)]
  selected_db_count2 <- db_count[db_count_colnames_group2]
  selected_db_meta2 <- db_meta[db_meta$Cell %in%  db_count_colnames_group2[2:length(db_count_colnames_group2)],]
  write.table(selected_db_meta2, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group2/cellphonedb_meta.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)
  write.table(selected_db_count2, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group2/cellphonedb_count.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)

  #Dataframe group_3
  finding3 <- paste(group3_sample[1],"|",group3_sample[2],"|",group3_sample[3],"|",group3_sample[4],"|",group3_sample[5],"|",group3_sample[6],"|Gene",sep = "")
  db_count_colnames_group3 <- db_count_colnames[grepl(finding3,db_count_colnames)]
  selected_db_count3 <- db_count[db_count_colnames_group3]
  selected_db_meta3 <- db_meta[db_meta$Cell %in%  db_count_colnames_group3[2:length(db_count_colnames_group3)],]
  write.table(selected_db_meta3, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group3/cellphonedb_meta.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)
  write.table(selected_db_count3, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group3/cellphonedb_count.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)

  #Dataframe group_4
  finding4 <- paste(group4_sample[1],"|",group4_sample[2],"|",group4_sample[3],"|",group4_sample[4],"|",group4_sample[5],"|",group4_sample[6],"|Gene",sep = "")
  db_count_colnames_group4 <- db_count_colnames[grepl(finding4,db_count_colnames)]
  selected_db_count4 <- db_count[db_count_colnames_group4]
  selected_db_meta4 <- db_meta[db_meta$Cell %in%  db_count_colnames_group4[2:length(db_count_colnames_group4)],]
  write.table(selected_db_meta4, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group4/cellphonedb_meta.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)
  write.table(selected_db_count4, paste('~/TOFA/plots/cellphone_output/input/',cell1,"_",cell2,"_group4/cellphonedb_count.txt",sep=""), sep = "\t",row.names = FALSE, quote = FALSE)
...
for (dir in directories) {
  ...
  stripped_dir <- sub("^/home/mmoro/TOFA/plots/cellphone_output/output/", "", dir)
  ...
  if(grepl("group1", dir)){
    ...
    significant_means_dir <- paste(dir,"/out/significant_means.txt",sep = "")
    significant_means <- read.delim(significant_means_dir, header = TRUE)
    cb_group1 <- obtain_cell_phone(directory,pvalues,significant_means)
  ...
  if(grepl("group2", dir)){
    ...
    significant_means_dir <- paste(dir,"/out/significant_means.txt",sep = "")
    significant_means <- read.delim(significant_means_dir, header = TRUE)
    cb_group2 <- obtain_cell_phone(directory,pvalues,significant_means)
  ...
  if(grepl("group3", dir)){
    ...
    significant_means_dir <- paste(dir,"/out/significant_means.txt",sep = "")
    significant_means <- read.delim(significant_means_dir, header = TRUE)
    cb_group3 <- obtain_cell_phone(directory,pvalues,significant_means)
  ...
  if(grepl("group4", dir)){
    ...
    significant_means_dir <- paste(dir,"/out/significant_means.txt",sep = "")
    significant_means <- read.delim(significant_means_dir, header = TRUE)
    cb_group4 <- obtain_cell_phone(directory,pvalues,significant_means)
...
cell_phone_group1 <- df_filtered[!duplicated(df_filtered[, c("Cluster_1", "Cluster_2", "id_cp_interaction","Subset_1","Subset_2")]), ]
cell_phone_group1 <- cell_phone_group1[-nrow(cell_phone_group1),]

file_path1 <- "~/TOFA/plots/cellphone_output/results/cell_phone1.xlsx"
...
cell_phone_group2 <- df_filtered[!duplicated(df_filtered[, c("Cluster_1", "Cluster_2", "id_cp_interaction","Subset_1","Subset_2")]), ]
cell_phone_group2 <- cell_phone_group2[-nrow(cell_phone_group2),]

file_path2 <- "~/TOFA/plots/cellphone_output/results/cell_phone2.xlsx"
...
cell_phone_group3 <- df_filtered[!duplicated(df_filtered[, c("Cluster_1", "Cluster_2", "id_cp_interaction","Subset_1","Subset_2")]), ]
cell_phone_group3 <- cell_phone_group3[-nrow(cell_phone_group3),]

file_path3 <- "~/TOFA/plots/cellphone_output/results/cell_phone3.xlsx"
...
cell_phone_group4 <- df_filtered[!duplicated(df_filtered[, c("Cluster_1", "Cluster_2", "id_cp_interaction","Subset_1","Subset_2")]), ]
cell_phone_group4 <- cell_phone_group4[-nrow(cell_phone_group4),]

file_path4 <- "~/TOFA/plots/cellphone_output/results/cell_phone4.xlsx"

# Export the dataframe to Excel
write.xlsx(cell_phone_group4, file_path4, rowNames = FALSE)

#Calling function
final_cellphone <- fuse_all_cellphones(cell_phone_group1,cell_phone_group2,cell_phone_group3,cell_phone_group4)

#Path
file_path_final <- "~/TOFA/plots/cellphone_output/results/final_cell_phone.xlsx"
