...
todas$annotation_intermediate <-
  gsub(" ", "_", todas$annotation_intermediate)
...
for (com in 1:6) {
  for (anot in annotation) {
    ...
    cells_cond1 <-
      todas@meta.data[todas@meta.data$condition == cond1, ]$cell_name
    cells_cond2 <-
      todas@meta.data[todas@meta.data$condition == cond2, ]$cell_name
